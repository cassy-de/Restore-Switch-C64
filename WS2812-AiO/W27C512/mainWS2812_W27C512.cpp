...
See pin definition on the "inside" of the ic pinout below 
              ...
              ------------   |
                            / \
                    +5V o    ^    o GND
  WS2812       XTAL/D10 o 0    10 o D0/A0  -> OUT KERNAL "PIN A15" (J2)     
               XTAL/D9  o 1     9 o D1/A1  -> OUT KERNAL "PIN A14" (J15)    
...
int mode2show = 0;        // # of selected "show program" after delay
int maxmode2show = 3;     // maximun number of "show programs" to choose from 
unsigned int delay2show = 6000;  // after 6000ms, start the show (rainbow)
int delayeffect = 0;   // counter to slow down the effects
int rainbowCycles = 0;   // counter for the rainbow effect
int chasecnt = 0;        // counter for the chasing light effect

boolean doReset = false;  // if true -> exec a reset of C64
int doClick = 0;          // counter for clicks of restoe key
...
void chasinglight(uint32_t col) {
  for (uint16_t i=0; i < strip.numPixels(); i=i+3) {
      strip.setPixelColor(i+chasecnt, col);             // set one LED on every three LEDs
    }
    strip.show();
    for (uint16_t i=0; i < strip.numPixels(); i=i+3) {
      strip.setPixelColor(i+chasecnt, 0);               // and now turn the one off
    }
    delayeffect++;                // now we have to slow down the chasing lights
    if ( delayeffect > 10 ) {     // outer loop counts delayeffect up
      delayeffect = 0;
      chasecnt++;                        // inner loop to chase the lights
      if( chasecnt > 2 ) chasecnt = 0;
    }
}

void twinklestar(uint32_t col) {
    delayeffect++;                // now we have to slow down the twinkling stars
    if ( delayeffect > 50 ) {     // outer loop counts delayeffect up
      delayeffect = 0;
      for (uint16_t i=0; i < strip.numPixels(); i++) {
        if ( random(10) > 8 ) {
...
void doState() {    // kernal switcher - here's where the magic happens
  ...
  switch(state) {   // This is the version for the 512k W27C512 EEPROMs
    case 0:
      blue();
      digitalWrite(kernalPIN14, LOW);    
      digitalWrite(kernalPIN15, LOW); 
      break;
    case 1: 
      red();
      digitalWrite(kernalPIN14, HIGH);  
      digitalWrite(kernalPIN15, LOW);  
      break;
...
//  change below to <<state == 4>> for larger 512k EEPROMS
    ...
    switch(( millis() - restoreHoldTime )) {   // calculate the time of restore key being pressed
    case 50 ... 300:    doClick++; break;  
    case 800 ... 1999:  doReset = true; break;  
    }

    restoreHoldTime = millis();      // clear time stamp
  ...
  // in case of double clicking restore key -> choose next "show program" pattern
  if ( doClick == 2 ) {
    doClick = 0;
    mode2show++;
  ...
  if (millis() - lastStateSwitchTime > delay2show) { // Delay to Show Time (6s)
    ...
    switch (mode2show) {      // select, which "show program" to display
      ...
      case 1:
        ...
        break;
      case 2:
        chasinglight(strip.Color(200, 200, 200));
        break;
      case 3:
        twinklestar(strip.Color(255, 255, 255));
        break;
    }
  }
  delay(10); // 10ms
