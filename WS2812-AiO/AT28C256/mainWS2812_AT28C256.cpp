...
void doState() {    // kernal switcher - here's where the magic happens
  ...
  switch(state) {   // WARNING !!!! This is the version fpr the AT28C256 EEPROMs
    case 0:
      blue();
      digitalWrite(kernalPIN14, HIGH);  // WE must be STAY HIGH
      digitalWrite(kernalPIN15, LOW);   // Kernel: LOW Address
      break;
    case 1: 
      red();
      digitalWrite(kernalPIN14, HIGH);  // WE must be STAY HIGH
      digitalWrite(kernalPIN15, HIGH);  // Kernal: HIGH Address
      break;
